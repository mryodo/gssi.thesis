...
%In the case of \(2\)-Core, see examples on \Cref{fig:2-core}, the most sparse cases (assuming all the formed triangles are included in \( \mc K \)) are mosaic triangulations with \( \mathrm{deg} \ge 4\); the \(4\)-regular graph can be extended further, alternatively, each face of the \(2\)-core can be fractally substituted by shown structures.
%{\blav One can probably show that \( \mathrm{deg} v \ge 4\) is necessary, but who cares.} 
%Exemplary structures on \Cref{fig:2-core} imply various possible configurations for a  \(d\)-Core, \( d \ge 2 \), hence a search for \(d\)-Core inside \( \mc K \) is neither trivial, no computationally cheap.
